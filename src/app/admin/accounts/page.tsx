  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        <p className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-sc-text-dimmer">
                ...
                {suspended.map((p) => (
                  <tr key={p.id}>
                    <td data-label="Person">
                      ...
                      <p className="text-xs font-semibold text-sc-text-dimmer">{p.email}</p>
                    </td>
                    <td data-label="Joined" className="text-sm text-sc-text-dim">
                      {when(p.created_at)}
                    </td>
                    <td data-label="">
                ...
                {deleted.map((d) => (
                  <tr key={d.id}>
                    <td data-label="Person">
                      ...
                      <p className="text-xs font-semibold text-sc-text-dimmer">{d.email}</p>
                      {d.reason && (
                        <p className="mt-1 text-xs text-sc-text-dim">{d.reason}</p>
                      )}
                    </td>
                    <td data-label="Handle" className="text-sm text-sc-text-dim">
                      {d.username ? `@${d.username}` : "—"}
                    </td>
                    <td data-label="Orders kept" className="text-sm text-sc-text-dim">
                      {d.orders_kept}
                    </td>
                    <td data-label="Deleted" className="text-sm text-sc-text-dim">
                      {when(d.deleted_at)}
                    </td>
                  </tr>
                ))}
